...
//$pop=$_POST["pop"];
$cutoff=$_POST["fdr"];
//echo "$input[0] $input[1] $model $cutoff";
for($i=0;$i<2;$i++)
{
$keys[$i]=trim($input[$i]);
if(strlen($keys[$i])<2)
{
echo "Wrong gene names!";
exit;
}
}
//search database
...
$sql1 = "select * from result where(gene1 like '%".$keys[0]."%' and gene2 like '%".$keys[1]."%') or (gene1 like '%".$keys[1]."%' and gene2 like '%".$keys[0]."%')";
$index1=0;
